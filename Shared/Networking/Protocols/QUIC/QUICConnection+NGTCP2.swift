...
// The ngtcp2 C-library boundary for ``QUICConnection``: thin wrappers over the
// `ngtcp2_conn_*` / `ngtcp2_swift_*` entry points the connection core drives, so its own
// read/write/timer/stream logic names no ngtcp2 symbol. `conn` is the `ngtcp2_conn *`, owned by
// the connection and valid only on the bridge queue; all of these run there, checked by actor
// isolation. (The TLS/crypto callbacks live in ``QUICConnection`` +NGTCP2Callbacks; the reentrancy
// guard — ``enterConnHeld()`` — is in the connection core.)
extension QUICConnection {
    ...
    /// ngtcp2 copies the addrs internally, so the copies need only outlive the call. Kept here so
    /// the raw `sockaddr`/`ngtcp2_path` pointer juggling stays at the C boundary.

...
struct ConnectionGraphView: View {
    ...
    private static let trunkX: CGFloat = 26
    private static let branchX: CGFloat = 74
    private static let labelX: CGFloat = 98
    ...
    private static func center(of node: ConnectionGraphNode) -> CGPoint {
        CGPoint(
            x: node.isOnTurnBranch ? branchX : trunkX,
    ...
    private var edges: some View {
        ZStack(alignment: .topLeading) {
            edge(from: .vpnSetup, to: .vkAccess)      // ответвление
            edge(from: .vkAccess, to: .captcha)
            edge(from: .captcha, to: .tunnelSetup)
            edge(from: .tunnelSetup, to: .connected)  // мерж
            edge(from: .vpnSetup, to: .connected)     // прямой ствол
            edge(from: .connect, to: .vpnSetup)
        }
    }
    ...
    private func dot(for node: ConnectionGraphNode) -> some View {
        ...
        ZStack {
            ...
            Circle()
                ...
                .shadow(
                    color: state == .off ? .clear : Self.reachedColor.opacity(state == .current ? 0.75 : 0.45),
    ...
    private func fill(for state: ConnectionNodeState) -> Color {
        state == .off ? Color.white.opacity(0.06) : Self.reachedColor
    }
    ...
    private func labelColor(for state: ConnectionNodeState) -> Color {
        switch state {
        case .off:     return .white.opacity(0.45)
...
#Preview("Все этапы") {
    ScrollView {
        VStack(alignment: .leading, spacing: 24) {
            ForEach(
                [ConnectionStage.idle, .connecting, .vpnSetup, .vkAccess,
                 .captcha, .tunnelSetup, .connectedViaTurn, .connectedDirect],
                id: \.self
            ) { stage in
